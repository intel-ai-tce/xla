...
module attributes {"triton_gpu.target" = "cuda:80", "triton_gpu.num-warps" = 4 : i32} {
  ...
  tt.func @sparse_blocked_to_mma_ampere(%A: tensor<64x32xf16, #lhs>, %B: tensor<64x64xf16, #rhs>, %meta: tensor<64x4xi16, #blocked>) -> tensor<64x64xf32, #blocked> {
    ...
    // CHECK: %[[OUT:.+]] = xla_triton_ext.sparse_dot %[[LHS]], %[[RHS]], %[[ACC]], %[[META]] : {{.+}} -> tensor<64x64xf32, #[[$MMA]]>
    %D = xla_triton_ext.sparse_dot %A, %B, %C, %meta : tensor<64x32xf16, #lhs> meta tensor<64x4xi16, #blocked> * tensor<64x64xf16, #rhs> -> tensor<64x64xf32, #blocked>
