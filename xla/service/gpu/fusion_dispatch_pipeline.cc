...
bool IsSlowLoopTransposeFusion(const HloFusionInstruction* fusion) {
  ...
  return transpose_dimensions[result_minormost_dim_in_operand] == rank - 1 &&
  ...
  bool is_bitcasted_transpose_with_power_of_two_minor_dim = ::xla::Match(
      ...
      m::Transpose(
          m::Bitcast(m::Parameter())
              .WithPredicate([&device_description](
                                 const HloInstruction* bitcast) {
                int64_t minor_dim_size = bitcast->shape().dimensions_minor(0);
                int64_t minor_dim_size_bytes =
                    primitive_util::ByteWidth(bitcast->shape().element_type()) *
                    minor_dim_size;
                return minor_dim_size_bytes >=
                           device_description
                               .dram_to_l2_transaction_size_bytes() ||
                       llvm::PowerOf2Ceil(minor_dim_size) == minor_dim_size;
              })));
